...
#define HARDWARE_PROFILE_LOW_PIN_COUNT_USB_DEVELOPMENT_KIT_H
    ...
    /** LED ************************************************************/
    #define mInitAllLEDs()      LATC &= 0xF5; TRISC &= 0xF5;
	...
	//Set high to low edge trigger and enable interrupt
	#define mTwiPinConf()	 INTCON2bits.INTEDG0 = 0; INTCONbits.INT0IE = 1; INTCON2bits.INTEDG2 = 0; INTCON3bits.INT2IE = 1;
	//Clear INT0 interrupt flag
	#define mClrInt0F() INTCONbits.INT0IF = 0;
	//Clear INT2 interrupt flag
	#define mClrInt2F() INTCON3bits.INT2IF = 0;
	//Select digital input
	#define mClrAnsel() 	ANSEL = 0;
	/*Enable timer, set prescale to 4, use internal clock*/
	#define mTimer0Con() T0CON = 0x81;
	//Clear Timer0 interrupt flag
	#define mClrTm0Init() INTCONbits.TMR0IF = 0;
	//Set preload values to overflow every 1ms
	//Each instruction will take invert of 12Mhz times the scaler 4
	//This yields 3.33*10^-7 per instruction
	//1ms will take 3000 cycles (1*10^-3/3.33*10^-7)
